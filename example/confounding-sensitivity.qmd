...
- [<i class="fas fa-table"></i> `mosquito_nets.csv`](/files/data/generated_data/mosquito_nets.csv)
- [<i class="fas fa-table"></i> `catholic.dta`](/files/data/external_data.catholic.dta)
...
school_data <- read_stata(here::here("files", "data", "external_data", "catholic.dta")) %>% 
  zap_labels() %>%
  zap_formats() %>%
  mutate(catholic_f = factor(catholic, labels = c("Public", "Catholic"))) %>% 
  mutate(inc8 = recode(faminc8,
...
```{r partial-net-r2, warning=FALSE}
library(rsq)
...
PHEW that was a lot of background information, but it's useful and important, so. Let's finally adjust for confounders and estimate causal effects in both the mosquito net and Catholic schools examples. We'll use [inverse probability weighting](/example/matching-ipw.qmd#inverse-probability-weighting) just because—we could also use matching or other techniques, but IPW is fun. Consult [the IPW guide](/example/matching-ipw.qmd#inverse-probability-weighting) for complete details about everything happening here.
...
tidy(model_net_ate, conf.int = TRUE) %>% 
  filter(term == "netTRUE") %>% 
  pull(conf.high) %>% 
  tip_coef(outcome_association = 5)
```
...
plot_stuff <- tidy(model_net_ate, conf.int = TRUE) %>% 
  filter(term == "netTRUE") %>% 
  pull(conf.high) %>%
  tip_coef(smd = seq(0.05, 3, by = 0.05), verbose = FALSE)
...
tidy(model_net_ate, conf.int = TRUE) %>% 
  filter(term == "netTRUE") %>% 
  pull(conf.high) %>%
  tip_coef(smd = 0.5, outcome_association = -0.3)
```
...
tidy(model_school_ate, conf.int = TRUE) %>% 
  ...
  pull(conf.low) %>%
  tip_coef(outcome_association = 1.6)
```
...
tidy(model_school_ate, conf.int = TRUE) %>% 
  filter(term == "catholic") %>% 
  pull(conf.low) %>%
  tip_coef(outcome_association = 0.8)
```
...
tidy(model_school_ate, conf.int = TRUE) %>% 
  filter(term == "catholic") %>% 
  pull(conf.low) %>%
  tip_coef(smd = 0.3, outcome_association = 0.4)
```
...
<iframe src="https://www.youtube.com/embed/p3dfHj6ki68" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen="" frameborder="0"></iframe>
</div>
...
```{r sense-school-model, warning=FALSE}
